...
export class MonitoringOpsStack extends cdk.Stack {
    constructor(scope, id, props) {
        ...
        const contactsAbandonedAlarm = contactsAbandonedMetric.createAlarm(this, `ContactsAbandonedAlarm-${props.envName}`, {
            alarmName: `MiniConnect-ContactsAbandoned-${props.envName}`,
            alarmDescription: "Contacts abandoning in queue — check agent availability and queue wait times.",
            threshold: 5,
            evaluationPeriods: 1,
            comparisonOperator: cloudwatch.ComparisonOperator.GREATER_THAN_OR_EQUAL_TO_THRESHOLD,
            treatMissingData: cloudwatch.TreatMissingData.NOT_BREACHING,
        });
        contactsAbandonedAlarm.addAlarmAction(new cloudwatch_actions.SnsAction(opsTopic));
        const contactsInQueueAlarm = contactsInQueueMetric.createAlarm(this, `ContactsInQueueAlarm-${props.envName}`, {
            alarmName: `MiniConnect-ContactsInQueue-${props.envName}`,
            alarmDescription: "Queue depth elevated — contacts waiting longer than expected.",
            threshold: 10,
            evaluationPeriods: 2,
            comparisonOperator: cloudwatch.ComparisonOperator.GREATER_THAN_OR_EQUAL_TO_THRESHOLD,
            treatMissingData: cloudwatch.TreatMissingData.NOT_BREACHING,
        });
        contactsInQueueAlarm.addAlarmAction(new cloudwatch_actions.SnsAction(opsTopic));
