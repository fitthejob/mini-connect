...
async function resolveBucketName() {
    ...
    const bucket = outputs.find((o) => o.ExportName === `LambdaArtifactBucketName-${environment}`)?.OutputValue;
    if (!bucket) {
        throw new Error(`Could not resolve bucket name from MiniConnect-S3 stack. Is it deployed?`);
    }
    return bucket;
}
async function stageLambda(fileName, bucketName) {
    ...
    if (!response.VersionId) {
        throw new Error(`S3 did not return a VersionId for ${baseName}.zip — ensure versioning is enabled on the bucket`);
    }
...
async function main() {
    ...
    if (lambdaTarget === "all") {
        ...
        if (!fileName) {
            throw new Error(`Unknown lambda target "${lambdaTarget}". Valid targets: ${Object.keys(lambdaRegistry).join(", ")}, all`);
        }
        await stageLambda(fileName, bucketName);
    }
}
