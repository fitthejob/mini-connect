...
import { ConnectClient, UpdateContactFlowContentCommand, ListContactFlowsCommand, } from "@aws-sdk/client-connect";
...
async function getSandboxFlowIds(connect, instanceId) {
    const response = await connect.send(new ListContactFlowsCommand({ InstanceId: instanceId }));
    ...
    if (!inbound?.Id || !queue?.Id) {
        throw new Error(`Sandbox flows not found. Create them once:\n` +
...
async function validateFlow(connect, instanceId, sandboxId, content) {
    try {
...
async function main() {
    ...
    const artifacts = [supportArtifact, ...fullRender.artifacts.filter((a) => a.key !== "supportQueueExperience")];
    let passed = 0;
    let failed = 0;
