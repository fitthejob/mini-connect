...
function validateGetParticipantInputAction(action: FlowAction): void {
  ...
  throw new Error(
    `Action "${action.id}" of type "GetParticipantInput" requires either the Lex-backed mode (Text plus LexV2Bot), the stored-input mode (StoreInput with proven DTMF configuration), or the touchtone-buffer mode (EnableDTMFBuffer).`,
  );
}
...
function validateProfileResponseData(action: FlowAction): void {
  ...
  for (const [key, value] of Object.entries(responseData)) {
    if (key.trim().length === 0 || typeof value !== "boolean") {
      throw new Error(
        `Action "${action.id}" of type "${action.type}" requires every ProfileResponseData entry to use a non-empty key and boolean value.`,
      );
    }
  }
}
