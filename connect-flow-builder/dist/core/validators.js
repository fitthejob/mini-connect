...
const CHECK_METRIC_DATA_METRIC_TYPES = new Set([
...
const STAFFING_METRIC_TYPES = new Set([
    "NumberOfAgentsAvailable",
    "NumberOfAgentsStaffed",
    "NumberOfAgentsOnline",
]);
...
const FLOW_LOGGING_BEHAVIORS = new Set([
    "Enabled",
    "Disabled",
]);
const QUEUE_CHANNELS = new Set(["Voice", "Chat"]);
const OUTBOUND_CALL_STATUS_OPERANDS = new Set([
    "CallAnswered",
    "VoicemailBeep",
    "VoicemailNoBeep",
    "NotDetected",
]);
const CHECK_VOICE_ID_OPTIONS = new Set([
    "enrollmentStatus",
    "voiceAuthentication",
    "fraudDetection",
]);
...
const PREVIOUS_CONTACT_PARTICIPANT_STATES = new Set([
    "AgentOnHold",
    "CustomerOnHold",
    "OffHold",
]);
const PERSISTENT_CONTACT_REHYDRATION_TYPES = new Set([
    "ENTIRE_PAST_SESSION",
    "FROM_SEGMENT",
]);
const LOAD_CONTACT_CONTENT_TYPES = new Set([
    "EmailMessage",
]);
...
const LOOP_OPERANDS = new Set(["ContinueLooping", "DoneLooping"]);
const CONTACT_EVENT_HOOK_TYPES = new Set([
    "AgentHold",
    "AgentWhisper",
    "CustomerHold",
    "CustomerQueue",
...
function validateCompareAction(action) {
...
function validateConnectParticipantWithLexBotAction(action) {
    ...
    if (hasLexV2Bot === hasLexBot) {
        throw new Error(`Action "${action.id}" of type "ConnectParticipantWithLexBot" requires exactly one of LexV2Bot or LexBot.`);
    }
    validateSinglePromptVariant(action, "ConnectParticipantWithLexBot");
    if (hasLexV2Bot) {
        const lexV2Bot = action.parameters.LexV2Bot;
        if (typeof lexV2Bot.AliasArn !== "string" || lexV2Bot.AliasArn.trim().length === 0) {
            throw new Error(`Action "${action.id}" of type "ConnectParticipantWithLexBot" requires LexV2Bot.AliasArn to be a non-empty string.`);
        }
    }
    if (hasLexBot) {
        const lexBot = action.parameters.LexBot;
        for (const key of ["Name", "Region", "Alias"]) {
            if (typeof lexBot[key] !== "string" || lexBot[key].trim().length === 0) {
                throw new Error(`Action "${action.id}" of type "ConnectParticipantWithLexBot" requires LexBot.${key} to be a non-empty string.`);
            }
        }
    }
    ...
    if (timeout !== undefined) {
        if (!isObject(timeout) || !("Text" in timeout)) {
            throw new Error(`Action "${action.id}" of type "ConnectParticipantWithLexBot" requires LexTimeoutSeconds to be an object with Text.`);
        }
        const textValue = timeout.Text;
        if (!((typeof textValue === "number" && Number.isInteger(textValue) && textValue > 0)
            || (typeof textValue === "string" && textValue.trim().length > 0))) {
            throw new Error(`Action "${action.id}" of type "ConnectParticipantWithLexBot" requires LexTimeoutSeconds.Text to be a positive integer or a non-empty JSONPath string.`);
        }
    }
    for (const condition of action.transitions?.conditions ?? []) {
        if (condition.condition.operator !== "Equals") {
            throw new Error(`Action "${action.id}" of type "ConnectParticipantWithLexBot" only supports Equals conditions.`);
        }
    }
}
function validateEndFlowExecutionAction(action) {
    if (Object.keys(action.parameters).length > 0) {
        throw new Error(`Action "${action.id}" of type "EndFlowExecution" does not accept parameters.`);
    }
}
function validateDistributeByPercentageAction(action) {
    if (Object.keys(action.parameters).length > 0) {
        throw new Error(`Action "${action.id}" of type "DistributeByPercentage" does not accept parameters.`);
    }
    const conditions = action.transitions?.conditions ?? [];
    if (conditions.length === 0) {
        throw new Error(`Action "${action.id}" of type "DistributeByPercentage" requires at least one condition.`);
    }
    let previousThreshold = 0;
    for (const condition of conditions) {
        if (condition.condition.operator !== "NumberLessThan") {
            throw new Error(`Action "${action.id}" of type "DistributeByPercentage" only supports NumberLessThan conditions.`);
        }
        const operand = Number(condition.condition.operands[0]);
        if (!Number.isInteger(operand) || operand < 1 || operand > 100) {
            throw new Error(`Action "${action.id}" of type "DistributeByPercentage" requires thresholds to be integers between 1 and 100.`);
        }
        if (operand <= previousThreshold) {
            throw new Error(`Action "${action.id}" of type "DistributeByPercentage" requires thresholds to increase strictly in ascending order.`);
        }
        previousThreshold = operand;
    }
}
function validateCheckOutboundCallStatusAction(action) {
    if (Object.keys(action.parameters).length > 0) {
        throw new Error(`Action "${action.id}" of type "CheckOutboundCallStatus" does not accept parameters.`);
    }
    const conditions = action.transitions?.conditions ?? [];
    if (conditions.length === 0) {
        throw new Error(`Action "${action.id}" of type "CheckOutboundCallStatus" requires at least one condition.`);
    }
    for (const condition of conditions) {
        if (condition.condition.operator !== "Equals") {
            throw new Error(`Action "${action.id}" of type "CheckOutboundCallStatus" only supports Equals conditions.`);
        }
        const operand = condition.condition.operands[0];
        if (!OUTBOUND_CALL_STATUS_OPERANDS.has(operand)) {
            throw new Error(`Action "${action.id}" of type "CheckOutboundCallStatus" uses unsupported operand "${operand}".`);
        }
    }
    requireErrorTypes(action, ["NoMatchingError"]);
}
function validateCreateTaskAction(action) {
    ...
    if (hasDelaySeconds && hasScheduledTime) {
        throw new Error(`Action "${action.id}" of type "CreateTask" cannot define both DelaySeconds and ScheduledTime.`);
    }
    if (hasDelaySeconds) {
        const value = action.parameters.DelaySeconds;
        if (!Number.isInteger(value) || value < 1 || value > 518400) {
            throw new Error(`Action "${action.id}" of type "CreateTask" requires DelaySeconds to be an integer between 1 and 518400.`);
        }
    }
    if (hasScheduledTime) {
...
function validateCreatePersistentContactAssociationAction(action) {
    const rehydrationType = requireNonEmptyStringParameter(action, "RehydrationType");
    if (!PERSISTENT_CONTACT_REHYDRATION_TYPES.has(rehydrationType)) {
        throw new Error(`Action "${action.id}" of type "CreatePersistentContactAssociation" requires RehydrationType to be ENTIRE_PAST_SESSION or FROM_SEGMENT.`);
    }
    requireNonEmptyStringParameter(action, "SourceContactId");
    requireErrorTypes(action, ["NoMatchingError"]);
}
function validateLoadContactContentAction(action) {
    const contentType = requireNonEmptyStringParameter(action, "ContentType");
    if (!LOAD_CONTACT_CONTENT_TYPES.has(contentType)) {
        throw new Error(`Action "${action.id}" of type "LoadContactContent" requires ContentType to be EmailMessage.`);
    }
    requireErrorTypes(action, ["NoMatchingError"]);
}
function validateGetParticipantInputAction(action) {
...
function validateSetTouchtoneBufferBehaviorAction(action) {
    requireErrorTypes(action, ["NoMatchingError"]);
    if ((action.transitions?.conditions?.length ?? 0) > 0) {
        throw new Error(`Action "${action.id}" of type "GetParticipantInput" does not support conditional transitions in the currently implemented Set Touchtone Buffer Behavior mode.`);
    }
    const enableDtmfBuffer = requireNonEmptyStringParameter(action, "EnableDTMFBuffer");
    if (!isConnectTrueFalseString(enableDtmfBuffer)) {
        throw new Error(`Action "${action.id}" of type "GetParticipantInput" requires EnableDTMFBuffer to be True, False, true, or false in the currently implemented Set Touchtone Buffer Behavior mode.`);
    }
    for (const unsupportedKey of [
        ...
        if (unsupportedKey in action.parameters) {
            throw new Error(`Action "${action.id}" of type "GetParticipantInput" does not support ${unsupportedKey} in the currently implemented Set Touchtone Buffer Behavior mode.`);
        }
    }
    if (normalizeConnectTrueFalseString(enableDtmfBuffer) === "True") {
        if ("StoreInput" in action.parameters) {
            throw new Error(`Action "${action.id}" of type "GetParticipantInput" only supports StoreInput in Set Touchtone Buffer Behavior stop-and-clear mode.`);
        }
        if ("InputEncryption" in action.parameters) {
            throw new Error(`Action "${action.id}" of type "GetParticipantInput" only supports InputEncryption in Set Touchtone Buffer Behavior stop-and-clear mode.`);
        }
        return;
    }
    if ("StoreInput" in action.parameters) {
        ...
        if (!isConnectTrueFalseString(storeInput)) {
            throw new Error(`Action "${action.id}" of type "GetParticipantInput" requires StoreInput to be True, False, true, or false in the currently implemented Set Touchtone Buffer Behavior mode.`);
        }
        if (normalizeConnectTrueFalseString(storeInput) !== "True") {
            throw new Error(`Action "${action.id}" of type "GetParticipantInput" only supports StoreInput = True in the currently implemented Set Touchtone Buffer Behavior mode.`);
        }
    }
    if ("InputEncryption" in action.parameters) {
        if (!("StoreInput" in action.parameters)) {
            throw new Error(`Action "${action.id}" of type "GetParticipantInput" requires StoreInput when InputEncryption is used in the currently implemented Set Touchtone Buffer Behavior mode.`);
        }
        const inputEncryption = requireObjectParameter(action, "InputEncryption");
        requireNestedNonEmptyString(action, inputEncryption, "InputEncryption", "EncryptionKeyId", "GetParticipantInput");
        requireNestedNonEmptyString(action, inputEncryption, "InputEncryption", "Key", "GetParticipantInput");
    }
}
function validateStoreCustomerInputAction(action) {
    requireErrorTypes(action, ["NoMatchingError"]);
    if ((action.transitions?.conditions?.length ?? 0) > 0) {
        throw new Error(`Action "${action.id}" of type "GetParticipantInput" does not support conditional transitions in the currently implemented Store customer input mode.`);
    }
    const inputTimeLimit = requireNonEmptyStringParameter(action, "InputTimeLimitSeconds");
    validatePositiveIntegerOrIntegerString(action, inputTimeLimit, "InputTimeLimitSeconds", "GetParticipantInput");
    const dtmfConfiguration = requireObjectParameter(action, "DTMFConfiguration");
    const disableCancelKey = requireNestedNonEmptyString(action, dtmfConfiguration, "DTMFConfiguration", "DisableCancelKey", "GetParticipantInput");
    if (disableCancelKey !== "True" && disableCancelKey !== "False") {
        throw new Error(`Action "${action.id}" of type "GetParticipantInput" requires DTMFConfiguration.DisableCancelKey to be True or False in the currently implemented Store customer input mode.`);
    }
    const interdigitTimeLimit = requireNestedNonEmptyString(action, dtmfConfiguration, "DTMFConfiguration", "InterdigitTimeLimitSeconds", "GetParticipantInput");
    validatePositiveIntegerOrIntegerString(action, interdigitTimeLimit, "DTMFConfiguration.InterdigitTimeLimitSeconds", "GetParticipantInput");
    ...
    if (!isObject(customValidation)) {
        throw new Error(`Action "${action.id}" of type "GetParticipantInput" requires InputValidation.CustomValidation in the currently implemented Store customer input mode.`);
    }
    const maximumLength = requireNestedNonEmptyString(action, customValidation, "InputValidation.CustomValidation", "MaximumLength", "GetParticipantInput");
    validatePositiveIntegerOrIntegerString(action, maximumLength, "InputValidation.CustomValidation.MaximumLength", "GetParticipantInput");
}
function validateCreateCaseAction(action) {
...
function validateCreateCustomerProfileAction(action) {
    validateProfileRequestDataHasEntries(action);
    validateProfileResponseData(action);
    requireErrorTypes(action, ["NoMatchingError"]);
}
function validateEvaluateDataTableValuesAction(action) {
...
function validateCheckHoursOfOperationAction(action) {
    const conditions = action.transitions?.conditions ?? [];
    if (conditions.length !== 2) {
        throw new Error(`Action "${action.id}" of type "CheckHoursOfOperation" requires exactly two conditions.`);
    }
    const operandValues = new Set();
    for (const condition of conditions) {
        if (condition.condition.operator !== "Equals") {
            throw new Error(`Action "${action.id}" of type "CheckHoursOfOperation" only supports Equals conditions.`);
        }
        const operand = condition.condition.operands[0];
        if (operand !== "True" && operand !== "False") {
            throw new Error(`Action "${action.id}" of type "CheckHoursOfOperation" only supports Equals True and Equals False conditions.`);
        }
        operandValues.add(operand);
    }
    if (!operandValues.has("True") || !operandValues.has("False")) {
        throw new Error(`Action "${action.id}" of type "CheckHoursOfOperation" requires one True condition and one False condition.`);
    }
}
function validateCheckMetricDataAction(action) {
    ...
    if (hasQueueId && hasAgentId) {
        throw new Error(`Action "${action.id}" of type "CheckMetricData" cannot define both QueueId and AgentId.`);
    }
    if (conditions.length === 0) {
        throw new Error(`Action "${action.id}" of type "CheckMetricData" requires at least one condition.`);
    }
    ...
    for (const error of action.transitions?.errors ?? []) {
        if (error.errorType === "NoMatchingCondition") {
            throw new Error(`Action "${action.id}" of type "CheckMetricData" does not support NoMatchingCondition for staffing metrics.`);
        }
    }
}
function validateStaffingMetricCondition(action, condition) {
    if (condition.condition.operator !== "NumberGreaterThan"
        || condition.condition.operands[0] !== "0") {
        throw new Error(`Action "${action.id}" of type "CheckMetricData" only supports NumberGreaterThan 0 conditions for staffing metrics.`);
    }
}
function validateMessageParticipantIterativelyAction(action) {
...
function validateLoopAction(action) {
    ...
    if (typeof loopCount === "number") {
        if (!Number.isInteger(loopCount) || loopCount < 0 || loopCount > 100) {
            throw new Error(`Action "${action.id}" of type "Loop" requires LoopCount to be an integer between 0 and 100.`);
        }
    }
    else if (!(typeof loopCount === "string" && loopCount.trim().length > 0)) {
        throw new Error(`Action "${action.id}" of type "Loop" requires LoopCount to be an integer between 0 and 100 or a non-empty JSONPath string.`);
    }
    const conditions = action.transitions?.conditions ?? [];
    if (conditions.length !== 2) {
        throw new Error(`Action "${action.id}" of type "Loop" requires exactly two conditions.`);
    }
    const operands = new Set();
    for (const condition of conditions) {
        if (condition.condition.operator !== "Equals") {
            throw new Error(`Action "${action.id}" of type "Loop" only supports Equals conditions.`);
        }
        const operand = condition.condition.operands[0];
        if (!LOOP_OPERANDS.has(operand)) {
            throw new Error(`Action "${action.id}" of type "Loop" only supports ContinueLooping and DoneLooping operands.`);
        }
        operands.add(operand);
    }
    if (!operands.has("ContinueLooping") || !operands.has("DoneLooping")) {
        throw new Error(`Action "${action.id}" of type "Loop" requires one ContinueLooping condition and one DoneLooping condition.`);
    }
}
function validateInvokeFlowModuleAction(action) {
    requireNonEmptyStringParameter(action, "FlowModuleId");
}
function validateGetMetricDataAction(action) {
    const hasQueueId = "QueueId" in action.parameters;
    const hasAgentId = "AgentId" in action.parameters;
    if (hasQueueId && hasAgentId) {
        throw new Error(`Action "${action.id}" of type "GetMetricData" cannot define both QueueId and AgentId.`);
    }
    if (hasQueueId) {
    ...
    if ("QueueChannel" in action.parameters) {
        ...
        if (!QUEUE_CHANNELS.has(queueChannel)) {
            throw new Error(`Action "${action.id}" of type "GetMetricData" requires QueueChannel to be Voice or Chat.`);
        }
    }
}
function validateGetCaseAction(action) {
    validateLowercaseBooleanStringParameter(action, "LinkContactToCase");
...
function validateGetCustomerProfileAction(action) {
...
function validateGetCustomerProfileObjectAction(action) {
    const requestData = requireObjectParameter(action, "ProfileRequestData");
...
function validateListDataTableValuesAction(action) {
    requireNonEmptyStringParameter(action, "DataTableId");
    requireErrorTypes(action, ["NoMatchingError"]);
...
function validateAuthenticateParticipantAction(action) {
    const cognitoConfiguration = requireObjectParameter(action, "CognitoConfiguration");
    for (const key of ["UserPoolArn", "AppClientId"]) {
        if (typeof cognitoConfiguration[key] !== "string"
            || cognitoConfiguration[key].trim().length === 0) {
            throw new Error(`Action "${action.id}" of type "AuthenticateParticipant" requires CognitoConfiguration.${key} to be a non-empty string.`);
        }
    }
    const customerProfilesConfiguration = requireObjectParameter(action, "CustomerProfilesConfiguration");
    if (typeof customerProfilesConfiguration.ObjectTypeName !== "string"
        || customerProfilesConfiguration.ObjectTypeName.trim().length === 0) {
        throw new Error(`Action "${action.id}" of type "AuthenticateParticipant" requires CustomerProfilesConfiguration.ObjectTypeName to be a non-empty string.`);
    }
    const timeLimitMinutes = action.parameters.TimeLimitMinutes;
    const numericTimeLimit = typeof timeLimitMinutes === "number"
        ? timeLimitMinutes
    ...
    if (!Number.isInteger(numericTimeLimit)
        || numericTimeLimit <= 0) {
        throw new Error(`Action "${action.id}" of type "AuthenticateParticipant" requires TimeLimitMinutes to be a positive integer or integer string.`);
    }
    for (const condition of action.transitions?.conditions ?? []) {
        if (condition.condition.operator !== "Equals") {
            throw new Error(`Action "${action.id}" of type "AuthenticateParticipant" only supports Equals conditions.`);
        }
        const operand = condition.condition.operands[0];
        if (!AUTHENTICATE_PARTICIPANT_OPERANDS.has(operand)) {
            throw new Error(`Action "${action.id}" of type "AuthenticateParticipant" only supports the OptedOut condition operand.`);
        }
    }
    requireErrorTypes(action, ["TimeLimitExceeded", "NoMatchingError"]);
}
function validateAssociateContactToCustomerProfileAction(action) {
    const requestData = requireObjectParameter(action, "ProfileRequestData");
    if (typeof requestData.ProfileId !== "string"
        || requestData.ProfileId.trim().length === 0
...
function validateCheckVoiceIdAction(action) {
    const option = action.parameters.CheckVoiceIdOption;
    if (!option || !CHECK_VOICE_ID_OPTIONS.has(option)) {
        throw new Error(`Action "${action.id}" of type "CheckVoiceId" requires CheckVoiceIdOption to be one of ${[...CHECK_VOICE_ID_OPTIONS].join(", ")}.`);
    }
    const conditions = action.transitions?.conditions ?? [];
    if (conditions.length === 0) {
        throw new Error(`Action "${action.id}" of type "CheckVoiceId" requires at least one condition.`);
    }
    const allowedOperands = getAllowedCheckVoiceIdOperands(option);
    for (const condition of conditions) {
        if (condition.condition.operator !== "Equals") {
            throw new Error(`Action "${action.id}" of type "CheckVoiceId" only supports Equals conditions.`);
        }
        const operand = condition.condition.operands[0];
        if (!allowedOperands.has(operand)) {
            throw new Error(`Action "${action.id}" of type "CheckVoiceId" does not support operand "${operand}" for option "${option}".`);
        }
    }
    requireErrorTypes(action, ["NoMatchingError"]);
}
function validateCompleteOutboundCallAction(action) {
    if ("CallerId" in action.parameters) {
        ...
        if (typeof callerId.Number !== "string" || callerId.Number.trim().length === 0) {
            throw new Error(`Action "${action.id}" of type "CompleteOutboundCall" requires CallerId.Number to be a non-empty string when provided.`);
        }
    }
    ...
    if ("ConnectionTimeLimitSeconds" in action.parameters) {
        requireIntegerInRangeParameter(action, "ConnectionTimeLimitSeconds", 1, 600, "an integer between 1 and 600");
    }
}
function validateCreateCallbackContactAction(action) {
    const queueId = action.parameters.QueueId;
    const agentId = action.parameters.AgentId;
    if (queueId !== undefined && agentId !== undefined) {
        throw new Error(`Action "${action.id}" of type "CreateCallbackContact" cannot define both QueueId and AgentId.`);
    }
    ...
    requireIntegerInRangeParameter(action, "InitialCallDelaySeconds", 1, 259200, "an integer between 1 and 259200");
    requireIntegerInRangeParameter(action, "MaximumConnectionAttempts", 1, Number.MAX_SAFE_INTEGER, "a positive integer");
    requireIntegerInRangeParameter(action, "RetryDelaySeconds", 1, 259200, "an integer between 1 and 259200");
...
function validateCreateWisdomSessionAction(action) {
    requireNonEmptyStringParameter(action, "WisdomAssistantArn");
    requireErrorTypes(action, ["NoMatchingError"]);
}
function validateDequeueContactAndTransferToQueueAction(action) {
    const queueId = action.parameters.QueueId;
    const agentId = action.parameters.AgentId;
    if ((queueId === undefined) === (agentId === undefined)) {
...
function validateEndFlowModuleExecutionAction(action) {
    if (Object.keys(action.parameters).length > 0) {
        throw new Error(`Action "${action.id}" of type "EndFlowModuleExecution" does not accept parameters.`);
    }
}
function validateGetCalculatedAttributesForCustomerProfileAction(action) {
    const requestData = requireObjectParameter(action, "ProfileRequestData");
    if (typeof requestData.ProfileId !== "string"
        || requestData.ProfileId.trim().length === 0) {
...
function validateStartOutboundChatContactAction(action) {
    ...
    if (typeof sourceEndpoint.Address !== "string"
        || sourceEndpoint.Address.trim().length === 0
        || sourceEndpoint.Type !== "CONNECT_PHONENUMBER_ARN") {
        throw new Error(`Action "${action.id}" of type "StartOutboundChatContact" requires SourceEndpoint to use a non-empty Address and Type "CONNECT_PHONENUMBER_ARN".`);
    }
    if (typeof destinationEndpoint.Address !== "string"
        || destinationEndpoint.Address.trim().length === 0
        || destinationEndpoint.Type !== "TELEPHONE_NUMBER") {
        throw new Error(`Action "${action.id}" of type "StartOutboundChatContact" requires DestinationEndpoint to use a non-empty Address and Type "TELEPHONE_NUMBER".`);
    }
    requireNonEmptyStringParameter(action, "ContactFlowArn");
    if (action.parameters.ContactSubtype !== "connect:SMS") {
        throw new Error(`Action "${action.id}" of type "StartOutboundChatContact" only supports ContactSubtype "connect:SMS".`);
    }
    if ("InitialSystemMessage" in action.parameters) {
        ...
        if (typeof initialSystemMessage.Content !== "string"
            || initialSystemMessage.Content.trim().length === 0) {
            throw new Error(`Action "${action.id}" of type "StartOutboundChatContact" requires InitialSystemMessage.Content to be a non-empty string when provided.`);
        }
    }
    if ("RelatedContact" in action.parameters
        && action.parameters.RelatedContact !== "CURRENT") {
        throw new Error(`Action "${action.id}" of type "StartOutboundChatContact" only supports RelatedContact "CURRENT".`);
    }
    requireErrorTypes(action, ["NoMatchingError"]);
}
function validateStartVoiceIdStreamAction(action) {
    if (Object.keys(action.parameters).length > 0) {
        throw new Error(`Action "${action.id}" of type "StartVoiceIdStream" does not accept parameters.`);
    }
    requireErrorTypes(action, ["NoMatchingError"]);
}
function validateUpdatePreviousContactParticipantStateAction(action) {
...
function requireNonEmptyStringParameter(action, key) {
...
function requireMetricTypeParameter(action) {
    const metricType = requireNonEmptyStringParameter(action, "MetricType");
    if (!CHECK_METRIC_DATA_METRIC_TYPES.has(metricType)) {
        throw new Error(`Action "${action.id}" of type "CheckMetricData" uses unsupported MetricType "${metricType}".`);
    }
    return metricType;
}
function validateTransferToFlowAction(action) {
    requireNonEmptyStringParameter(action, "ContactFlowId");
}
function validateTransferContactToQueueAction(action) {
    if (Object.keys(action.parameters).length > 0) {
        throw new Error(`Action "${action.id}" of type "TransferContactToQueue" does not accept parameters.`);
    }
    requireErrorTypes(action, [
        "QueueAtCapacity",
        "NoMatchingError",
...
function validateTransferParticipantToThirdPartyAction(action) {
    requireNonEmptyStringParameter(action, "ThirdPartyPhoneNumber");
    const connectionTimeLimit = requireNonEmptyStringParameter(action, "ThirdPartyConnectionTimeLimitSeconds");
    validatePositiveIntegerOrIntegerString(action, connectionTimeLimit, "ThirdPartyConnectionTimeLimitSeconds", "TransferParticipantToThirdParty");
    validateTrueFalseField(action, requireNonEmptyStringParameter(action, "ContinueFlowExecution"), "ContinueFlowExecution");
    if ("ThirdPartyDTMFDigits" in action.parameters) {
        requireNonEmptyStringParameter(action, "ThirdPartyDTMFDigits");
    }
    if ("CallerId" in action.parameters) {
        const callerId = requireObjectParameter(action, "CallerId");
        requireNestedNonEmptyString(action, callerId, "CallerId", "Name", "TransferParticipantToThirdParty");
        requireNestedNonEmptyString(action, callerId, "CallerId", "Number", "TransferParticipantToThirdParty");
    }
    requireErrorTypes(action, [
        "CallFailed",
...
function validateUpdateContactEventHooksAction(action) {
...
function validateUpdateContactMediaProcessingAction(action) {
...
function validateUpdateContactMediaStreamingBehaviorAction(action) {
...
function validateUpdateContactRecordingAndAnalyticsBehaviorAction(action) {
...
function validateUnTagContactAction(action) {
...
function validateUpsertDataTableValuesAction(action) {
...
function validateUpdateContactCallbackNumberAction(action) {
...
function validateUpdateContactDataAction(action) {
...
function validateUpdateContactTargetQueueAction(action) {
    const queueId = action.parameters.QueueId;
    const agentId = action.parameters.AgentId;
    if ((queueId === undefined) === (agentId === undefined)) {
...
function validateUpdateContactTextToSpeechVoiceAction(action) {
...
function validateUpdateFlowAttributesAction(action) {
    const flowAttributes = action.parameters.FlowAttributes;
    if (!isObject(flowAttributes) || Object.keys(flowAttributes).length === 0) {
        throw new Error(`Action "${action.id}" of type "UpdateFlowAttributes" requires FlowAttributes to contain at least one entry.`);
    }
    for (const [key, value] of Object.entries(flowAttributes)) {
        if (key.trim().length === 0 || typeof value !== "string" || value.trim().length === 0) {
            throw new Error(`Action "${action.id}" of type "UpdateFlowAttributes" requires every FlowAttributes entry to use a non-empty string key and value.`);
        }
    }
}
function validateUpdateFlowLoggingBehaviorAction(action) {
...
function validateUpdateCustomerProfileAction(action) {
    validateProfileRequestDataHasEntries(action);
    validateProfileResponseData(action);
    requireErrorTypes(action, ["NoMatchingError"]);
}
function validateWaitAction(action) {
    ...
    const requiredOperands = new Set(["WaitCompleted", ...events]);
    const actualOperands = new Set();
    for (const condition of conditions) {
        if (condition.condition.operator !== "Equals") {
        ...
        if (!requiredOperands.has(operand)) {
            throw new Error(`Action "${action.id}" of type "Wait" uses unsupported condition operand "${operand}".`);
        }
        actualOperands.add(operand);
    }
    for (const operand of requiredOperands) {
        if (!actualOperands.has(operand)) {
            throw new Error(`Action "${action.id}" of type "Wait" requires a condition for "${operand}".`);
        }
    }
}
function validateWaitTimeout(action) {
    const timeout = action.parameters.TimeoutSeconds;
    if (typeof timeout === "number") {
        if (!Number.isInteger(timeout) || timeout <= 0 || timeout > 604800) {
            throw new Error(`Action "${action.id}" of type "Wait" requires TimeoutSeconds to be a positive integer no greater than 604800.`);
        }
        return;
    }
    ...
    throw new Error(`Action "${action.id}" of type "Wait" requires TimeoutSeconds to be a positive integer or a non-empty JSONPath string.`);
}
function validateWaitEvents(action) {
    ...
    for (const event of events) {
        if (typeof event !== "string" || !WAIT_EVENTS.has(event)) {
            throw new Error(`Action "${action.id}" of type "Wait" uses unsupported event "${String(event)}".`);
        }
    }
    return events;
}
function validateRangeStringParameter(action, key, min, max) {
    if (!(key in action.parameters)) {
        return;
    }
    const value = requireNonEmptyStringParameter(action, key);
...
function validateStringMapParameter(action, key) {
    if (!(key in action.parameters)) {
        return;
    }
    const value = action.parameters[key];
...
function validateStringArrayParameter(action, key) {
    if (!(key in action.parameters)) {
        return;
    }
    const value = action.parameters[key];
...
function requireArrayParameter(action, key, actionType) {
    const value = action.parameters[key];
...
function requireNestedArray(action, parent, parentName, key, actionType) {
...
function validateArrayLengthRange(action, value, fieldName, actionType, min, max) {
    if (value.length < min || value.length > max) {
        throw new Error(`Action "${action.id}" of type "${actionType}" requires ${fieldName} to contain between ${min} and ${max} entries.`);
    }
}
function validateNestedStringArray(action, parent, key, parentName, actionType) {
...
function validateNamedPrimaryValues(action, parent, key, nameKey, parentName, actionType) {
...
function validateLowercaseBooleanStringParameter(action, key) {
    const value = requireNonEmptyStringParameter(action, key);
    if (value !== "true" && value !== "false") {
        throw new Error(`Action "${action.id}" of type "${action.type}" requires ${key} to be "true" or "false".`);
    }
}
function requireObjectParameter(action, key) {
    const value = action.parameters[key];
    if (!isObject(value)) {
...
function requireNestedNonEmptyString(action, parent, parentName, key, actionType) {
    const value = parent[key];
    if (typeof value !== "string" || value.trim().length === 0) {
...
function validatePositiveIntegerOrIntegerString(action, value, fieldName, actionType) {
...
function validateCustomerProfileSearchCriteria(action, criteria) {
...
function validateMessageLoopContent(action, message) {
    if (!isObject(message)) {
...
function validateUserDefinedTagEntry(action, key, value, actionType) {
...
function validateSinglePromptVariant(action, actionType) {
    const promptKeys = ["PromptId", "Text", "SSML", "Media"].filter((key) => action.parameters[key] !== undefined);
    if (promptKeys.length !== 1) {
        throw new Error(`Action "${action.id}" of type "${actionType}" requires exactly one of PromptId, Text, SSML, or Media.`);
    }
...
function validateTrueFalseField(action, value, fieldName) {
    if (value !== "True" && value !== "False") {
        throw new Error(`Action "${action.id}" of type "${action.type}" requires ${fieldName} to be True or False.`);
    }
}
function isConnectTrueFalseString(value) {
    return value === "True"
        || value === "False"
        || value === "true"
        || value === "false";
}
function normalizeConnectTrueFalseString(value) {
    return value.toLowerCase() === "true" ? "True" : "False";
}
function getAllowedCheckVoiceIdOperands(option) {
    switch (option) {
...
function validateCompleteOutboundCallVoiceConnector(action, voiceConnector) {
    if (voiceConnector.VoiceConnectorType !== "ChimeConnector") {
        throw new Error(`Action "${action.id}" of type "CompleteOutboundCall" only supports VoiceConnectorType "ChimeConnector".`);
    }
    for (const key of ["VoiceConnectorArn", "FromUser", "ToUser"]) {
        if (typeof voiceConnector[key] !== "string"
            || voiceConnector[key].trim().length === 0) {
            throw new Error(`Action "${action.id}" of type "CompleteOutboundCall" requires VoiceConnector.${key} to be a non-empty string.`);
        }
    }
    if ("UserToUserInformation" in voiceConnector
        ...
        throw new Error(`Action "${action.id}" of type "CompleteOutboundCall" requires VoiceConnector.UserToUserInformation to be a non-empty string when provided.`);
    }
}
function requireIntegerInRangeParameter(action, key, minimum, maximum, expectedDescription) {
    const value = action.parameters[key];
    if (typeof value !== "number"
        || !Number.isInteger(value)
        || value < minimum
        || value > maximum) {
        throw new Error(`Action "${action.id}" of type "${action.type}" requires "${key}" to be ${expectedDescription}.`);
    }
    return value;
}
function requireErrorTypes(action, errorTypes) {
...
function validateMediaObject(action, media, actionType) {
    if (!isObject(media)) {
...
function validateVoiceBehavior(action, voiceBehavior) {
...
function validateChatBehavior(action, chatBehavior) {
...
function validateScreenRecordingBehavior(action, screenRecordingBehavior) {
...
function isLikelyJsonPath(value) {
    return /^\$(\.|\[)/.test(value);
}
function validateUpdateRoutingCriteriaAction(action) {
...
function validateRoutingCriteriaStep(action, step) {
...
function validateRoutingCriteriaAttributeCondition(action, condition) {
...
function validateShowViewAction(action) {
